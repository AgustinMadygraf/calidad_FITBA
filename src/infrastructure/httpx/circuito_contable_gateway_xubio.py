"""
Path: src/infrastructure/httpx/circuito_contable_gateway_xubio.py
"""
...
CIRCUITO_CONTABLE_PATH = "/API/1.1/circuitoContableBean"
CIRCUITO_CONTABLE_ID_KEYS = ("circuitoContable_id", "ID", "id")
_LIST_CACHE_PROVIDER, _ = providers_for_module(
    namespace="circuito_contable", with_item_cache=False
)
# Compatibility alias for tests and debug tooling.
_GLOBAL_LIST_CACHE = _LIST_CACHE_PROVIDER.store


class XubioCircuitoContableGateway(CircuitoContableGateway):
    def __init__(
        self,
        ...
        if list_cache_ttl is None:
            list_cache_ttl = read_cache_ttl("XUBIO_CIRCUITO_CONTABLE_LIST_TTL")
    ...
    def list(self) -> List[Dict[str, Any]]:
        ...
        if cached is not None:
            return cached
        url = self._url(CIRCUITO_CONTABLE_PATH)
        items = list_items(
            url=url, timeout=self._timeout, label="circuitos contables", logger=logger
        )
    ...
    def get(self, circuito_contable_id: int) -> Optional[Dict[str, Any]]:
        items = self.list()
        for item in items:
            if match_any_id(item, circuito_contable_id, CIRCUITO_CONTABLE_ID_KEYS):
                return item
        return None

    def _get_cached_list(self) -> Optional[List[Dict[str, Any]]]:
        cached = _LIST_CACHE_PROVIDER.get(
            CIRCUITO_CONTABLE_PATH, ttl=self._list_cache_ttl
        )
        if cached is not None:
            logger.info(
                "Xubio lista circuitos contables: cache hit (%d items)", len(cached)
            )
        return cached

    def _store_cache(self, items: List[Dict[str, Any]]) -> None:
        _LIST_CACHE_PROVIDER.set(
            CIRCUITO_CONTABLE_PATH, items, ttl=self._list_cache_ttl
        )
