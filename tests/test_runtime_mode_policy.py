...
def test_non_prod_builds_vendedor_gateway_with_read_cache(monkeypatch):
    monkeypatch.setenv("IS_PROD", "false")

    gateway = deps.get_vendedor_gateway()

    assert isinstance(gateway, XubioVendedorGateway)
    assert gateway._list_cache_ttl > 0


def test_prod_builds_vendedor_gateway_without_read_cache(monkeypatch):
    monkeypatch.setenv("IS_PROD", "true")
