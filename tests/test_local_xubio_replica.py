...
def test_xubio_like_unit_measure_crud(client) -> None:
    create_resp = client.post(
        "/API/1.1/UnidadMedidaBean",
        json={
            "ID": 101,
            "codigo": "KG",
            "nombre": "Kilogramo",
    ...
    assert created["codigo"] == "KG"
    assert created["nombre"] == "Kilogramo"

    list_resp = client.get("/API/1.1/UnidadMedidaBean")
    assert list_resp.status_code == 200
    items = list_resp.json()
    assert any(item["ID"] == 101 for item in items)

    patch_resp = client.patch(
        "/API/1.1/UnidadMedidaBean/101",
        json={"nombre": "Kilo"},
    )
    assert patch_resp.status_code == 200
    assert patch_resp.json()["nombre"] == "Kilo"

    get_resp = client.get("/API/1.1/UnidadMedidaBean/101")
    assert get_resp.status_code == 200
    assert get_resp.json()["nombre"] == "Kilo"

    delete_resp = client.delete("/API/1.1/UnidadMedidaBean/101")
    assert delete_resp.status_code == 204
